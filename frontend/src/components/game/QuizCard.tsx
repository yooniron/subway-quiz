...
export const QuizCard: React.FC<QuizCardProps> = ({
    ...
    const answerPlaceholder = getAnswerPlaceholder();
    const textLength = answerPlaceholder.length;

    let dynamicFontSize = 'text-2xl sm:text-5xl md:text-6xl';
    if (textLength >= 8) {
        dynamicFontSize = 'text-lg sm:text-2xl md:text-3xl lg:text-4xl';
    } else if (textLength >= 5) {
        dynamicFontSize = 'text-xl sm:text-3xl md:text-4xl lg:text-5xl';
    }
    ...
    return (
        ...
        >
            ...
            <div className="relative flex items-start justify-between w-full px-1 sm:px-4 pt-10 sm:pt-12 pb-4 sm:pb-6 mt-4 sm:mt-6">
                {/* 5개 노드 중앙 축 통과 수평 트랙 선 */}
                <div 
                    className="absolute left-4 right-4 h-3.5 sm:h-5 -z-10 rounded-full transition-all duration-300 shadow-inner top-[58px] sm:top-[68px] -translate-y-1/2" 
                    style={{ backgroundColor: quiz.color_code }} 
                />
                
                <div className={`flex flex-col items-center w-1/5 min-w-0 transition-all ${isL2Visible ? 'opacity-100' : 'opacity-20 blur-xs'}`}>
                    <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full border-4 border-white bg-gray-950 flex items-center justify-center shadow-lg shrink-0">
                        <span className="w-3 h-3 rounded-full bg-white/70" />
                    </div>
                    <span className="mt-2 sm:mt-3 text-[11px] sm:text-xs md:text-sm font-bold text-center [word-break:keep-all] leading-tight w-full max-w-[80px] sm:max-w-none min-h-[32px] sm:min-h-[40px] flex items-start justify-center px-0.5">
                        {isL2Visible ? quiz.left_2 : '?'}
                    </span>
                </div>
                
                <div className={`flex flex-col items-center w-1/5 min-w-0 transition-all ${isL1Visible ? 'opacity-100' : 'opacity-20 blur-xs'}`}>
                    <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full border-4 border-white bg-gray-950 flex items-center justify-center shadow-lg shrink-0">
                        <span className="w-3 h-3 rounded-full bg-white/70" />
                    </div>
                    <span className="mt-2 sm:mt-3 text-[11px] sm:text-xs md:text-sm font-bold text-center [word-break:keep-all] leading-tight w-full max-w-[80px] sm:max-w-none min-h-[32px] sm:min-h-[40px] flex items-start justify-center px-0.5">
                        {isL1Visible ? quiz.left_1 : '?'}
                    </span>
                </div>

                {/* 중앙 정답 타겟 노드 + Pure CSS 지하철 객차 수평 주행 (Metro Train Car Component) */}
                <div className="flex flex-col items-center w-1/5 min-w-0 relative">
                    <div className="absolute -top-9 sm:-top-10 flex items-center justify-center gap-1">
                        {/* Pure CSS 지하철 객차 모듈 (창문 2개 + 노선 띠 + 헤드라이트) */}
                        <div
                            className="w-10 h-5 sm:w-14 sm:h-7 rounded-lg border-2 border-white flex items-center justify-between px-1 shadow-xl"
                            style={{ backgroundColor: quiz.color_code }}
                        >
                            <span className="w-1 h-1 sm:w-1.5 sm:h-1.5 rounded-full bg-yellow-300 shadow-sm" />
                            <div className="flex gap-0.5 sm:gap-1">
                                <span className="w-2 h-2 sm:w-3 sm:h-3 rounded-xs bg-white/80 border border-black/20" />
                                <span className="w-2 h-2 sm:w-3 sm:h-3 rounded-xs bg-white/80 border border-black/20" />
                            </div>
                        </div>
                    </div>
                    <div
                        className="w-9 h-9 sm:w-11 sm:h-11 rounded-full border-4 border-yellow-400 bg-white text-gray-950 font-black text-xs sm:text-base flex items-center justify-center shadow-[0_0_25px_rgba(250,204,21,0.8)] shrink-0"
                    >
                        ?
                    </div>
                    <span className="mt-2 sm:mt-3 text-[11px] sm:text-xs md:text-sm font-black text-yellow-400 tracking-wider text-center select-none">[ 정답 ]</span>
                </div>
