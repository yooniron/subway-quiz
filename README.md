...
<!-- RELEASE_LOGS_START -->
- feat: 🎮 캐치마인드 스타일 실시간 8인 파티룸 & 정답자 시크릿 채팅 다인전 모드 및 Supabase DB 마이그레이션 구현 (2026-08-28)
- fix: 🛡️ 로그인 유저 실시간 통계 클라우드 영구 저장 및 스마트 키별 병합(Math.max) 구현 (2026-08-27)
- feat: 📊 나의 플레이 상세 통계 대시보드(UserStatsModal) 및 TOP 3 선호 노선 집계 시스템 구현 (2026-08-27)
- fix: 🛡️ 연습모드 칭호 어뷰징 방지 및 게임 세션 중도 이탈 시 진행 정보 완전 초기화 (2026-08-12)
...
- fix: 모바일 및 저해상도 환경 UI 비율 유지, 승강장 LIVE/역명 동적 스케일링 및 자연스러운 줄바꿈(keep-all) 개선 (2026-08-07)
- fix: Safari/WebKit에서 QuizCard 무한 애니메이션+blur로 인한 프리즈 해결 (2026-08-05)
- fix: 연습모드 모바일 반응 지연 및 정답/오답 모달 깜빡임 수정 (2026-08-04)
