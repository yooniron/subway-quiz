...
import { PlusCircle, X, Check, Dices, Lock, Globe, KeyRound, MapPin } from 'lucide-react';
import { 
    SUBWAY_LINES, 
    REGION_TABS, 
    getLinesByRegion, 
...
export const CreateRoomModal: React.FC<CreateRoomModalProps> = ({
    ...
    const handleToggleCurrentRegion = () => {
        ...
        if (isCurrentRegionAllSelected) {
            ...
            setSelectedLineIds(remaining.length > 0 ? remaining : [1]);
        } else {
                ...
                <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto pr-1 flex flex-col gap-4">
                    ...
                    <div>
                        ...
                        <input
                            ...
                            className="w-full px-4 py-2.5 min-h-[42px] bg-gray-950 border border-gray-800 rounded-xl text-sm text-white placeholder-gray-600 focus:outline-none focus:border-amber-400 transition-colors"
                        />
                    </div>
                    ...
                    {/* 공개 / 비공개 설정 스위치 */}
                    <div className="bg-gray-950/80 border border-gray-800/80 rounded-2xl p-3.5 flex flex-col gap-2.5">
                    ...
                    <div>
                        ...
                        <div className="flex gap-1.5 mb-2.5">
                            <button
                                ...
                                className={`flex-1 py-1.5 px-2 rounded-xl border text-[11px] font-bold transition-all flex items-center justify-center gap-1 ${
                                    isAllSelected
                            ...
                            >
                                ...
                                {isAllSelected ? '전체 해제' : '전국 28개 전체 선택'}
                            </button>
                            ...
                            {activeRegion !== 'ALL' && (
                                <button
                                    ...
                                    className={`py-1.5 px-2 rounded-xl border text-[11px] font-bold transition-all flex items-center justify-center gap-1 ${
                            ...
                            <button
                                ...
                                className="py-1.5 px-2.5 bg-gray-950 border border-gray-800 hover:border-amber-400/40 text-amber-400 text-[11px] font-bold rounded-xl transition-all flex items-center justify-center gap-1"
                            >
                        ...
                        {/* 노선 그리드 */}
                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-1.5 max-h-[160px] overflow-y-auto p-1 bg-gray-950/60 rounded-2xl border border-gray-800/80">
                            {displayedLines.map((line) => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={`relative py-2 px-2 rounded-xl border text-xs font-black transition-all flex items-center justify-between overflow-hidden cursor-pointer ${
                                            isSelected
                    ...
                    <button
                        type="submit"
                        className="w-full mt-2 py-3.5 bg-gradient-to-r from-amber-400 to-yellow-500 hover:from-amber-500 hover:to-yellow-600 text-gray-950 font-black text-sm rounded-2xl shadow-xl shadow-amber-400/20 transition-transform active:scale-95 cursor-pointer shrink-0"
                    >
                        🏠 방 만들기 ➕ ({selectedLineIds.length}개 호선)
                    </button>
                </form>
            </div>
        </div>
